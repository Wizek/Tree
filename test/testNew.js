  ...
  stree.branch('announcers', function(stree) {
    ...
    stree.branch('DOM', function(stree) {
      ...
      stree.branch('initing', function(stree) {
        ...
        tree1._initDom()
        stree($('body > div#tree-top.tree-top > li > span.handle').length).eql(1)
        stree($('body > div#tree-top.tree-top > li > span.stamp').length).eql(1)
        stree($('body > div#tree-top.tree-top > li > span.summary').length).eql(1)
      ...
      stree.branch('branching 101', function(stree) {
        ...
        stree($(spec+' ul li.branch').length).eql(1)
        stree($(spec+' ul li.branch > span.handle').length).eql(1)
        stree($(spec+' ul li.branch > span.stamp').length).eql(1)
      ...
      stree.branch('branching 202', function(stree) {
        ...
        tree2.branch('name 1', function(tree2) {
          ...
          tree2.branch('naase 1.2', function(tree2) {
            tree2.branch('naasdasdme 1.2.1', function(tree2) {
              tree2.branch('naasdasdme 1.2.1', function(tree2) {
          ...
          stree(t.hasClass('waiting')).ok()
          stree(t.hasClass('expanded')).not.ok()
          stree(t.hasClass('running')).not.ok()
          stree(t.hasClass('failed')).not.ok()
          stree(t.hasClass('passed')).ok()
          stree(t.hasClass('done')).ok()
        })
        var t = $('#'+randName+' li.branch')
